...
G=load('../pressure_data/geometry.mat');

badname={'POBS1';'POBS4';'POBS16'};
...
for i=1:length(badname)
    ...
    while count<4
        ...
        if length(G.filename{id})==23
            load([POBS_dir(1).folder '/' G.filename{id}(1:5) '.mat'])
        else
        ...
        print(['../figures/gauge_assessments/' badname{i} '_vs_' G.filename{id}],'-dpng','-r300')
