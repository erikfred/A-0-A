% import_POBS_Y2.m
%
% Convert POBS data from pseudo-binary matfiles
%
...
srcdir='/Volumes/Spare/POBS-data_2023-2024'; % location of binary files
svdir='../processed_POBS_Y2'; % where to save formatted & decimated data

% easier to list these manually so I don't have to re-run all of them every single time
instruments={'POBS01','POBS02','POBS03','POBS04','POBS05','POBS07',...
    'POBS08','POBS11','POBS12','POBS13','POBS14','POBS16'};
...
for i=1:length(instruments)
    ...
    if strcmp(instruments{i},'POBS14')
        Logs(iG2:end,:)=[];
        iG2=[]; % the timestamp in the file is clearly wrong
    end
    ...
    for j=1:length(flist)
        ...
        while i1<length(apg_temp)
            ...
            if length(segt)>24*60*60
                warning('More samples than expected')
                keyboard
            elseif length(segt)~=length(segA)
                warning('Time and data arrays different lengths!')
                keyboard
            end
            ta=cat(1,ta,segt);
            a1=cat(1,a1,segA(:,1));
    ...
    % remove anomalous entries and note indices
    timecheck=flip(find(diff(tb)<0));
    if ~isempty(timecheck)
        for k=1:length(timecheck)
            warning('i = %d, j = %d. tb(%d) = %f manually removed. \n',i,j,timecheck(k),tb(timecheck(k)))
            tb(timecheck(k)+1)=[];
            b(timecheck(k)+1)=[];
            Tb(timecheck(k)+1)=[];
        end
    ...
    for j=1:length(flist)
        ...
        if strcmp(flist(j).name,'TAX00004.009')
            fwork='../processed_POBS_Y2/POBS09/TAX00004.009';
        end
        ...
        if ~isempty(tax_hold)
            % remove bad timestamps (if any) at starts of files
            ibadt=find(tax_temp(:,1)<tax_hold(end,1));
            tax_temp(ibadt,:)=[];
        ...
        while i1<length(tax_temp)
            ...
            if length(segt)>24*60*60
                warning('More samples than expected')
                keyboard
            elseif length(segt)~=length(segT)
                warning('Time and data arrays different lengths!')
                keyboard
            end
            tt=cat(1,tt,segt);
    ...
    save(['../processed_POBS_Y2/' instruments{i} '/' instruments{i} '.mat'],'ta','a1','a2','Ta1','Ta2','tb','b','Tb')
    save(['../processed_POBS_Y2/' instruments{i} '/' instruments{i} '_triax.mat'],'tt','xt','yt','zt','Tt')
    ...
    print(['../processed_POBS_Y2/' instruments{i} '/' instruments{i} '_pressure'],'-dpng','-r100')
    ...
    print(['../processed_POBS_Y2/' instruments{i} '/' instruments{i} '_acceleration'],'-dpng','-r100')
end
