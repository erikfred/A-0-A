% process_POBS.m
%
% Script to process 2023-2024 Hikurangi A-0-A data
%
...
for i=i_list'
    ...
    if any(isnan(data.a1))
        ...
        else % catch-all for unexpected cases
            warning(['NaNs in ' POBS_dir(i).name ' need closer inspection'])
            continue
        end
    end
    ...
    sdir=['/Volumes/ExtremeSSD/decimated/' POBS_dir(i).name];
    if ~exist(sdir,'dir')
        mkdir(sdir)
    end
    ...
    save(['../pressure_data_Y2/' POBS_dir(i).name],'data','dataDec','calInfoAll1','calInfoAll2','barInfoAll','-v7.3')
    ...
    if ~isempty(calInfo1.t0)
        ...
        print(['../figures_Y2/calibrations/' POBS_dir(i).name '_cal_summary'],'-dpng','-r300')
        % print(['../figures_Y2/calibrations/' POBS_dir(i).name '_cal_summary'],'-depsc','-vector')
        ...
        fh.PaperUnits='inches';
        fh.PaperPosition=[0 0 11 8.5];
        print(['../figures_Y2/calibrations/' POBS_dir(i).name '_barcor_difference'],'-dpng','-r300')
        % print(['../figures_Y2/calibrations/' POBS_dir(i).name '_barcor_difference'],'-depsc','-vector')
